...
var StyleSheetManager = class extends (_a = abstract.CorePlugin, _additionalRoots_dec = [state.reactive], _sourceRoot_dec = [state.derived], _targetRoot_dec = [state.derived], _roots_dec = [state.derived], _a) {
  ...
  get sourceRoot() {
    ...
    return utilities.getRoot((_a5 = source == null ? void 0 : source.element) != null ? _a5 : null);
  }
  get targetRoot() {
    var _a5;
    const { target } = this.manager.dragOperation;
    return utilities.getRoot((_a5 = target == null ? void 0 : target.element) != null ? _a5 : null);
  }
  get roots() {
...
function createResizeObserver(ctx) {
  return new ResizeObserver(() => {
    var _a5, _b2;
    const placeholderShape = new utilities.DOMRectangle(ctx.placeholder, {
    ...
    const fixedOffset = utilities.getFixedPositionOffset();
    ctx.styles.set(
      {
    ...
    ctx.dragOperation.shape = new utilities.DOMRectangle(ctx.feedbackElement);
  });
}
...
function runDropAnimation(ctx) {
  ...
  utilities.showPopover(ctx.feedbackElement);
  const [, runningAnimation] = (_a5 = utilities.getFinalKeyframe(
    ctx.feedbackElement,
  ...
  const current = new utilities.DOMRectangle(ctx.feedbackElement, options);
  const currentTranslate = (_c3 = utilities.parseTranslate(utilities.getComputedStyles(ctx.feedbackElement).translate)) != null ? _c3 : ctx.translate;
  const final = new utilities.DOMRectangle(target, options);
  const delta = geometry.Rectangle.delta(current, final, ctx.alignment);
  ...
  utilities.animateTransform({
    ...
    options: {
      duration: utilities.prefersReducedMotion(utilities.getWindow(ctx.feedbackElement)) ? 0 : ctx.moved || ctx.feedbackElement !== ctx.element ? duration : 0,
      easing
    }
  }).then(() => {
...
var _Feedback = class _Feedback extends (_a2 = abstract.Plugin, _overlay_dec = [state.reactive], _a2) {
  constructor(manager, options) {
    super(manager, options);
...
render_fn = function() {
  ...
  const reducedMotion = utilities.prefersReducedMotion(feedbackWindow);
  ...
  const restoreFocus = () => {
    ...
    if (utilities.isHTMLElement(focusTarget)) {
      focusTarget.focus();
    }
  };
  const cleanup = () => {
...
_b = (_dec2 = [state.reactive], utilities.ScrollDirection.Forward), _a3 = (_dec = [state.reactive], utilities.ScrollDirection.Reverse);
...
var Scroller = class extends (_a4 = abstract.CorePlugin, _autoScrolling_dec = [state.reactive], _a4) {
  constructor(manager) {
    super(manager);
...
var defaults2 = Object.freeze({
  activationConstraints(event, source) {
    ...
    if (pointerType === "mouse" && utilities.isElement(target) && (source.handle === target || ((_a5 = source.handle) == null ? void 0 : _a5.contains(target)))) {
      return void 0;
    }
...
var DragDropManager = class extends abstract.DragDropManager {
  constructor(input = {}) {
    const plugins = abstract.resolveCustomizable(input.plugins, defaultPreset.plugins);
      ...
      __spreadValues({
        effects: () => [
          ...
          () => {
            ...
            if (!manager2) return;
            const sensors = (_b3 = (_a6 = this.sensors) == null ? void 0 : _a6.map(abstract.descriptor)) != null ? _b3 : [
              ...manager2.sensors
            ];
...
Object.defineProperty(exports, "resolveCustomizable", {
  enumerable: true,
  get: function () { return abstract.resolveCustomizable; }
});
